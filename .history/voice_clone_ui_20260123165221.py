    ...
    with gr.Blocks(title="Qwen3-TTS Voice Clone Studio", theme=gr.themes.Soft()) as app:
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("🎨 Voice Design"):
                ...
                and the model will generate speech matching that description.
                """)
                ...
                with gr.Row():
                    with gr.Column(scale=2):
                        ...
                        design_generate_btn = gr.Button("🎨 Generate Designed Voice", variant="primary", size="lg")
                    ...
                    with gr.Column(scale=1):
                        ...
                        gr.Markdown("""
                        **Tips for voice design:**
                        - Be specific about age, gender, emotion
                        - Describe speaking style (fast/slow, loud/soft)
                        - Include accent or dialect if desired
                        - Mention emotional qualities (warm, cold, excited, calm)
                        """)
            ...
            # ============== TAB 3: Design → Clone ==============
            with gr.TabItem("🔄 Design → Clone"):
                gr.Markdown("""
                ### Design a Voice, Then Clone It
                
                **Workflow:** First, design a voice with natural language instructions. 
                Then, use that designed voice to generate new content - giving you style control 
                that pure voice cloning doesn't offer.
                """)

                with gr.Row():
                    # Left - Design the reference
                    with gr.Column(scale=1):
                        gr.Markdown("### 1️⃣ Design Reference Voice")
                        
                        dc_design_text = gr.Textbox(
                            label="Reference Text",
                            placeholder="Short text for the voice design (this creates the reference voice)...",
                            lines=2,
                            value="Hello, this is a sample of my voice for cloning purposes."
                        )

                        dc_instruct = gr.Textbox(
                            label="Voice Design Instructions",
                            placeholder="Describe the voice characteristics...",
                            lines=3
                        )

                        dc_ref_audio = gr.Audio(
                            label="Designed Reference (auto-generated)",
                            type="filepath",
                            interactive=False
                        )

                    # Right - Clone with that voice
                    with gr.Column(scale=1):
                        gr.Markdown("### 2️⃣ Generate with Cloned Voice")
                        
                        dc_clone_text = gr.Textbox(
                            label="Text to Generate",
                            placeholder="Enter the actual text you want spoken with the designed voice...",
                            lines=4
                        )

                        with gr.Row():
                            dc_language = gr.Dropdown(
                                choices=LANGUAGES,
                                value="English",
                                label="Language",
                                scale=2
                        ...
                        dc_generate_btn = gr.Button("🚀 Design & Clone", variant="primary", size="lg")
                        dc_status = gr.Textbox(label="Status", interactive=False)

                        dc_output_audio = gr.Audio(
                            label="Final Output",
                            type="filepath"
                        )

                # Design → Clone event handlers
                dc_generate_btn.click(
                    generate_design_then_clone,
                    inputs=[dc_design_text, dc_instruct, dc_clone_text, dc_language, dc_seed],
                    outputs=[dc_ref_audio, dc_output_audio, dc_status]
                )
