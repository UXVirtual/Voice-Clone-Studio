...
def load_voice_prompt(sample_name, expected_hash, device='cuda:0'):
    ...
    try:
        ...
        prompt_items = {}
        for key, value in cache_data['prompt'].items():
            if isinstance(value, torch.Tensor):
...
def trim_audio(audio_file, start_time, end_time):
    ...
    try:
        ...
        return str(temp_path), f"✅ Trimmed to {format_time(new_duration)} ({new_duration:.2f}s)"

    except Exception as e:
        return None, f"❌ Error trimming: {str(e)}"
    ...
    with gr.Blocks(title="Qwen3-TTS Voice Clone Studio", theme=gr.themes.Soft()) as app:
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("🎤 Voice Clone"):
                gr.Markdown("### 🎯 Select Voice Sample")

                sample_dropdown = gr.Dropdown(
                    choices=get_sample_choices(),
                    label="Voice Sample",
                    info="Select a voice to clone (manage samples in Prep Samples tab)"
                )

                with gr.Row():
                    refresh_samples_btn = gr.Button("🔄 Refresh", size="sm", scale=1)
                    play_sample_btn = gr.Button("▶️ Play Sample", size="sm", scale=2)
                ...
                with gr.Row():
                    ...
                    randomize_seed_btn = gr.Button("🎲 Random", size="sm", scale=1)
                ...
                def play_selected_sample(sample_name):
                    """Get the audio path for the selected sample."""
                    if not sample_name:
                        return None
                    samples = get_available_samples()
                    for s in samples:
            ...
            with gr.TabItem("🎛️ Prep Samples"):
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=2):
                        ...
                        with gr.Row():
                            ...
                            trim_btn = gr.Button("✂️ Trim", variant="primary")
                ...
                def load_sample_to_editor(sample_name):
                    ...
                    if not sample_name:
                        return None, "", "No sample selected", 0, 0
                    samples = get_available_samples()
                    for s in samples:
                        if s["name"] == sample_name:
                            duration = get_audio_duration(s["wav_path"])
                            info = f"Duration: {format_time(duration)} ({duration:.2f}s)"
                            return s["wav_path"], s["ref_text"], info, 0, duration
                    return None, "", "Sample not found", 0, 0

                load_sample_btn.click(
                    load_sample_to_editor,
                    inputs=[existing_sample_dropdown],
                    outputs=[prep_audio_input, transcription_output, prep_audio_info, trim_start, trim_end]
                )
                ...
                prep_audio_input.change(
                    ...
                    outputs=[prep_audio_info, trim_start, trim_end]
                )

                # Trim audio
                trim_btn.click(
                    trim_audio,
                    inputs=[prep_audio_input, trim_start, trim_end],
